...
func (_q *SyncTaskQuery) QueryInstrument() *InstrumentQuery {
...
func (_q *SyncTaskQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (_q *SyncTaskQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (_q *SyncTaskQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (_q *SyncTaskQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (_q *SyncTaskQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (_q *SyncTaskQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *SyncTaskQuery) WithInstrument(opts ...func(*InstrumentQuery)) *SyncTaskQuery {
	query := (&InstrumentClient{config: _q.config}).Query()
...
//		GroupBy(synctask.FieldInstrumentID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.SyncTask.Query().
//		Select(synctask.FieldInstrumentID).
//		Scan(ctx, &v)
...
func (_q *SyncTaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*SyncTask, error) {
	...
	_spec.Node.Schema = _q.schemaConfig.SyncTask
	...
	if query := _q.withInstrument; query != nil {
		if err := _q.loadInstrument(ctx, query, nodes, nil,
			func(n *SyncTask, e *Instrument) { n.Edges.Instrument = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SyncTaskQuery) loadInstrument(ctx context.Context, query *InstrumentQuery, nodes []*SyncTask, init func(*SyncTask), assign func(*SyncTask, *Instrument)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*SyncTask)
	for i := range nodes {
...
func (_q *SyncTaskQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(synctask.Table, synctask.Columns, sqlgraph.NewFieldSpec(synctask.FieldID, field.TypeUUID))
	_spec.From = _q.sql
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withInstrument != nil {
			_spec.Node.AddColumnOnce(synctask.FieldInstrumentID)
		}
	}
...
func (_q *SyncTaskQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1.Schema(_q.schemaConfig.SyncTask)
	ctx = internal.NewSchemaConfigContext(ctx, _q.schemaConfig)
