...
// The TimeframeFunc type is an adapter to allow the use of ordinary
// function as Timeframe mutator.
type TimeframeFunc func(context.Context, *ent.TimeframeMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TimeframeFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TimeframeMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TimeframeMutation", m)
}
